...
/** Each item is a flat offer document returned by GET /api/offers/my-activity */
interface ActivityItem {
  _id: string;
  postId: ActivityPost;   // populated post
  message: string;
...
const STATUS = {
  accepted: {
    ...
    bannerMsg:
      "Your offer was accepted! The post author wants to work with you.",
  },
  rejected: {
    label: "Rejected",
    icon: XCircle,
  ...
  return (
    ...
    >
      ...
      <div className="pt-5 px-5 pb-4 space-y-4">
        {/* ── Status banner ── */}
        <div
          className={`flex items-start gap-2.5 px-3.5 py-2.5 rounded-xl border text-[12px] leading-snug ${cfg.banner}`}
        >
        ...
        {/* ── Expand toggle ── */}
        {(item.answers?.length > 0 || post?.description?.length > 120) && (
          <button
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-[13px] text-zinc-500 mt-0.5">
            All posts you've responded to — track your offer status in real
            time.
          </p>
        </div>
        <button
        ...
        <div className="flex items-center gap-1.5 overflow-x-auto pb-0.5 scrollbar-none">
          {FILTER_TABS.map(({ key, label, icon: Icon }) => {
            ...
            return (
              ...
              >
                <Icon
                  className={`w-3 h-3 ${active ? "text-white" : "text-zinc-600"}`}
                />
                {label}
                <span
                  className={`text-[9px] font-bold px-1 rounded-full ${active ? "bg-white/20 text-white" : "bg-[#1a1a1e] text-zinc-600"}`}
                >
                  {counts[key]}
                </span>
              </button>
        ...
        <div className="flex flex-col items-center justify-center py-24 gap-4 text-center">
          ...
          <div>
            ...
            <p className="text-[12px] text-zinc-600 mt-1 max-w-xs">
              Browse the feed and submit an offer on a post to see it tracked
              here.
            </p>
          </div>
        </div>
