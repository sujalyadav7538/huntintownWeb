...
export default function PostCard({
  ...
  const currentUser = useAppSelector((s) => s.auth.currentUser) as User | null;
  const author = normalizeAuthor(post.author);
  const isAuthor = isAuthorOf(author, currentUser);
  const isUrgent =
        ...
        <div className="flex items-start justify-between gap-3">
          <div className="flex items-center gap-2.5 min-w-0">
            <button
              ...
              title={
                onViewProfile ? `View ${post.author.name}'s profile` : undefined
              }
            >
              <img
