...
export default function LoginModal({
  ...
  const [activeTab, setActiveTab] = useState<"signin" | "preset" | "forge">(
    "signin",
  );
    ...
    <div className="fixed inset-0 z-50 bg-black/85 backdrop-blur-md flex items-center justify-center p-4 overflow-y-auto">
      <div className="bg-[#121214] rounded-2xl border border-[#232327] max-w-sm w-full overflow-hidden text-zinc-100 shadow-2xl animate-in zoom-in-95 duration-250">
        ...
        <div className="p-5 text-center border-b border-[#1e1e21] bg-[#17171a] relative">
          ...
          <p className="text-[10px] text-zinc-400 font-semibold tracking-wide mt-0.5">
            Login to continue connection exchanges
          </p>
        </div>
          ...
          {activeTab === "signin" && (
            <form onSubmit={handleSimulatedSubmit} className="space-y-3.5">
              ...
              <div className="flex items-center gap-2 py-0.5 my-1">
                ...
                <span className="text-[9px] font-mono uppercase tracking-widest text-[#FF3F3F]">
              ...
              <div className="space-y-2.5">
                <div>
                  <label className="block text-[9px] font-mono tracking-wider font-bold text-zinc-500 uppercase mb-1">
                    Email
                  </label>
                ...
                <div>
                  <div className="flex justify-between items-center mb-1">
                    <label className="block text-[9px] font-mono tracking-wider font-bold text-zinc-500 uppercase">
                      Password
                    </label>
                    <span className="text-[8px] text-[#FF3F3F] hover:underline cursor-pointer">
                      Forgot password?
                    </span>
                  </div>
                  <div className="relative">
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <span className="block text-xs font-black text-white">
                    {currentUser.name}{" "}
                    <span className="font-normal text-[9px] text-[#FF3F3F]">
                      (Default Star)
                    </span>
                  </span>
                ...
                {currentUserId === currentUser.id && (
                  <Check className="w-4 h-4 text-[#FF3F3F]" />
                )}
              </div>

              {/* Mock items removed — use sign in with credentials */}
            </div>
          )}
